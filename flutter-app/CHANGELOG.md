...
- **Telefono nuovo → "Recupera i messaggi"**, tre passi nello stile del wizard di pairing: da chi recuperare ("Dal mio partner" o "Da un altro mio telefono"), QR da far inquadrare, recupero. La scelta viaggia nel QR: il telefono che aiuta rifiuta un invio con il ruolo sbagliato e spiega quale pulsante usare. Il QR contiene una richiesta cifrata per una chiave temporanea. **Sul telefono del partner → "Aiuta il partner a recuperare"** (o sul vecchio telefono dello stesso utente → **"Trasferisci a un nuovo telefono"**): inquadra il QR, conferma, e il certificato viene ricifrato per il telefono nuovo, che ricompone la famiglia con lo stesso ID e scarica i messaggi. Solo in presenza: la richiesta (id casuale a 256 bit) viaggia unicamente nel QR e scade dopo 10 minuti; il server vede solo blob cifrati.
- Il deposito viene creato anche per le coppie già accoppiate prima di questa versione (al primo avvio con la chat aperta) e cancellato con "Elimina tutto" o con l'unpair esplicito.
- Rimosso "Ripristino Partner" dalle Impostazioni (rifare il pairing con lo stesso partner è ora coperto dal recupero). Rimossi la scelta "backup cloud / manuale", la pagina Backup, "Verifica backup", il promemoria, il certificato da copiare/incollare/condividere e la pagina Ripristino: l'ipotesi "reinstallo l'app su un telefono già configurato" non aveva un recupero cloud affidabile (Block Store lo consegna solo nella configurazione iniziale del telefono, iCloud Keychain solo con sincronizzazione attiva) e il certificato manuale è un peso per l'utente. Resta, invisibile, la copia locale nel Block Store per la reinstallazione sullo stesso Android.
- L'app non genera più una coppia di chiavi RSA all'avvio: su un telefono reinstallato la chiave nuova rendeva i vecchi messaggi indecifrabili. Le chiavi si creano solo nel wizard "Nuovo pairing".
- Spiegazione in-app prima della richiesta dei permessi posizione, microfono e fotocamera (richiesta da Google Play). Vale anche per chi RICEVE una condivisione di posizione e apre la mappa, con un testo dedicato: la posizione serve solo per la distanza mentre la mappa è aperta; rifiutando si vede comunque il partner.
- **Fix: dopo un ripristino i due telefoni si spaiavano da soli.** Il listener della famiglia prendeva decisioni anche sugli snapshot della cache locale di Firestore: subito dopo un ripristino la cache conteneva solo il documento appena scritto, l'app leggeva "il partner se n'è andato", cancellava il proprio documento e il partner, vedendo la famiglia incompleta, si spaiava a sua volta. Ora gli snapshot da cache o con scritture in attesa vengono ignorati e la famiglia è considerata completa solo quando il server mostra entrambi i documenti.
- Il ripristino verifica il certificato contro la chat sul server prima di dichiararsi accoppiato: un certificato riferito a una chat che non esiste più viene rifiutato con una spiegazione. Il documento utente viene riscritto solo se manca o è diverso, e con merge (i token FCM/VoIP non vengono cancellati).
- **Fix: famiglia "fantasma" con 1 membro senza chiavi.** Dopo un incidente che cancellava i documenti utente, il salvataggio del token FCM (set con merge) ricreava il documento senza `my_public_key`: la famiglia risultava 1/2 per sempre e il ripristino rifiutava il certificato. Ora i documenti senza chiavi non contano come membri (listener, wizard e ripristino); il token FCM viene salvato sempre insieme alle chiavi; se il proprio documento manca mentre il partner è presente (o la chat ha ancora messaggi) l'app lo riscrive da sola; se manca quello del partner, il ripristino lo ricostruisce dal certificato, che contiene entrambe le chiavi.

### 📤 Condivisione verso TuyJo (link/foto da altre app)
- iOS: i video sono esclusi dal foglio di condivisione (l'estensione non li gestisce; dall'app si inviano come prima).
- **iOS: anche foto e documenti vengono inviati direttamente dall'estensione**, senza aprire l'app (iOS non permette a un'estensione di condivisione di aprire l'app che la ospita: prima compariva "Apri Tuijo"). L'estensione ridimensiona a 2048 px di lato massimo (l'app resta a 4096: l'estensione ha un limite di memoria di ~120 MB e la decodifica ridotta di ImageIO lavora a fattori 2/4/8, quindi 2048 è il massimo che evita di decodificare l'originale intero; JPEG q85, thumbnail lato corto 300 px, foto presa da Foto come file JPEG, un bitmap alla volta), cifra AES-256-GCM con chiave avvolta RSA-OAEP per entrambi, carica su Firebase Storage via REST e scrive il messaggio con gli allegati nello stesso formato dell'app. HUD "Invio in corso… n/N"; se manca la rete ripiega sulla coda come prima.
- iOS: Tuijo compare nel foglio di condivisione anche selezionando più foto da Foto. La regola di attivazione dell'estensione era nel formato a dizionario (conteggi per tipo), che con la selezione multipla di Foto fallisce; ora è un predicato: fino a 10 elementi, ciascuno immagine, link, testo o file.
