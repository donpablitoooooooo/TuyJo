...
- **Fix: dopo un ripristino i due telefoni si spaiavano da soli.** Il listener della famiglia prendeva decisioni anche sugli snapshot della cache locale di Firestore: subito dopo un ripristino la cache conteneva solo il documento appena scritto, l'app leggeva "il partner se n'è andato", cancellava il proprio documento e il partner, vedendo la famiglia incompleta, si spaiava a sua volta (chat vuota da entrambe le parti finché non si rifaceva il QR). Ora gli snapshot da cache o con scritture in attesa vengono ignorati e la famiglia è considerata completa solo quando il server mostra entrambi i documenti.
- Il ripristino (automatico o manuale) verifica il certificato contro la chat sul server prima di dichiararsi accoppiato: un backup vecchio o riferito a una chat rifatta viene rifiutato con una spiegazione ("rifate il pairing con i QR") invece di portare in una chat vuota o di far scattare sul telefono del partner la pulizia della famiglia. Il documento utente viene riscritto solo se manca o è diverso.
- **Fix: famiglia "fantasma" con 1 membro senza chiavi.** Dopo un incidente che cancellava i documenti utente, il salvataggio del token FCM (set con merge) ricreava il documento senza `my_public_key`: la famiglia risultava 1/2 per sempre, i messaggi di esempio restavano in chat e il Ripristino rifiutava il certificato ("chiave diversa") pur essendo corretto. Ora: i documenti senza chiavi non contano come membri (listener, wizard e ripristino); il token FCM viene salvato sempre insieme alle chiavi; se il proprio documento manca o è vuoto mentre il partner è presente (o la chat ha ancora messaggi) l'app lo riscrive da sola, senza rifare i QR; il ripristino non cancella più il token FCM dal documento. Se manca il documento del partner, il ripristino lo ricostruisce dal certificato (che contiene entrambe le chiavi): la famiglia torna 2/2 subito, senza che il partner debba aggiornare l'app o rifare i QR.
